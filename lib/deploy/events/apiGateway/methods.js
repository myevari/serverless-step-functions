...
module.exports = {
  ...
  compileMethods() {
    ...
    this.pluginhttpValidated.events.forEach((event) => {
      ...
      const authorizationType = event.http.useIAMAuth ? 'AWS_IAM' : 'NONE';
  ...
  getMethodIntegration(stateMachineName, customName, http) {
    const DEFAULT_MAPPING = '{"input": "$body","name": "$context.requestId","stateMachineArn":"';
    const COGNITO_MAPPING = '{"input": "{\\"cognitoIdentityId\\" : \\"$context.identity.cognitoIdentityId\\", \\"body\\": $body}","name": "$context.requestId","stateMachineArn":"'; // eslint-disable-line
    const MAPPING = http && http.useIAMAuth ? COGNITO_MAPPING : DEFAULT_MAPPING;
